...
void Button2::setButtonStateFunction(StateCallbackFunctionEx f, void* ctx) {
    get_state_cb_ex = f;
    get_state_ctx = ctx;
}
...
void* Button2::getContext() const {
  if (get_state_cb_ex != nullptr) {
    return get_state_ctx;
  } else {
    int ptr = -1;
    return &ptr;
  }
...
if (get_state_cb_ex != nullptr) {
    return get_state_cb_ex(get_state_ctx);
} else if (get_state_cb != nullptr) {
    return get_state_cb();
} else {
