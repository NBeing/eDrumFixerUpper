# e-drum-fixer upper
This project uses a MCU in order to replace the "brain" of an electronic drum kit. 

To do this we have to re-create piezo conditioning circuitry which will allow the MCU (see below). 
Many DIY electronic drum projects lack velocity sensitivity and I think it is because information on proper signal processing is hard to find. Here is my stab at it! 

The gist is to use a full wave rectifier and some voltage dividers to make the signal stay within the ADC's voltage range, as well as rectifying the output of the piezo to make the logic for reading how hard it was hit a bit easier and more consistent.

An external ADC is also useful as they do a better job of sampling the input.
